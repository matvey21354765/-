...
# Healthcheck убран намеренно: он мог провоцировать перезапуск контейнера
# в случае задержки поднятия дашборда. Статус живости бота отслеживаем
# через стартовые сообщения в Telegram (см. control_bot.py).
